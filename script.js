...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (partnershipForm) {
        // Auto-detect country and set default country code
        ...
        partnershipForm.addEventListener('submit', function(e) {
            ...
            if (data.country_code && data.phone && data.phone.trim()) {
                data.full_phone = data.country_code + ' ' + data.phone.trim();
            } else if (data.phone && data.phone.trim()) {
                data.full_phone = data.phone.trim();
            }
            
            // Validation
            const requiredFields = ['company_name', 'your_name', 'job_title', 'email', 'company_type', 'partnership_interest'];
            ...
            // Track form submission
            trackEvent('partnership_form_submit', { 
                company_name: data.company_name,
                company_type: data.company_type,
                partnership_interest: data.partnership_interest,
                has_phone: !!(data.phone && data.phone.trim()),
                country_code: data.country_code,
                theme: document.documentElement.getAttribute('data-theme') 
            });
            
            // Submit to Google Apps Script
            submitPartnershipForm(data)
                .then(response => {
                    if (response.success) {
                        // Show success popup instead of inline message
                        showSuccessPopup(
                            'Partnership Inquiry Sent!',
                            'Thank you for your partnership inquiry! We\'ll get back to you within 24 hours. You should also receive a confirmation email shortly.',
                            () => {
                        ...
                        setTimeout(() => setDefaultCountryCode(), 100);
                    } else {
                        showFormMessage(response.message || 'An error occurred while submitting your inquiry. Please try again.', 'error');
                    }
                })
                .catch(error => {
                    console.error('Form submission error:', error);
                    showFormMessage('An error occurred while submitting your inquiry. Please try again or contact us directly.', 'error');
                })
                .finally(() => {
                    // Reset button
...
// Submit partnership form to Google Apps Script
async function submitPartnershipForm(data) {
    ...
    try {
        ...
        console.log('Partnership form submission result:', result);
        return result;
    ...
    try {
        ...
        if (countryFromTimezone) {
            ...
            for (let option of options) {
                if (option.dataset.country === countryFromTimezone) {
                    countryCodeSelect.value = option.value;
                    console.log(`Auto-detected country: ${countryFromTimezone} (${option.value})`);
                    return;
                }
            }
        }
        ...
        console.log('Using default country code: +1 (US)');
        
    } catch (error) {
        console.warn('Failed to auto-detect country code:', error);
        // Fallback to US
        countryCodeSelect.value = '+1';
    }
}
